...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbors = [neighbor for neighbor in possible_neighbors if 0 <= neighbor[0] < self.height and 0 <= neighbor[1] < self.width]

        new_sentence = Sentence(neighbors, count)
        self.knowledge.append(new_sentence)

        # Task 4: Mark additional cells as safe or as mines
        for sentence in self.knowledge:
            known_mines = sentence.known_mines()
            known_safes = sentence.known_safes()

            # Mark additional cells as mines or safes
            self.mines.update(known_mines)
            self.safes.update(known_safes)

        # Task 5: Add new sentences to the AI's knowledge base
